...
results <- merge( results, psi_0, by = c("period","regime","effect_size", "coarsen"))
# 
# 
...
conditions <- c("data","period","regime","n", "learners", "coarsen")
...
# throw out perf for the DGP estimate (seems to be a bug)
perf <- perf[!(data=="MC"&learners=="glm_true")]
...
for(n_i in sample_sizes){
  ...
  p <- ggplot(perf_pe[n==n_i], aes(x=regime, y= value,
                                   ...
                                   color = factor(data):factor(coarsen), fill = factor(data):factor(coarsen)))+
...
for(n_i in sample_sizes){
  ...
  p <- ggplot(bias[n==n_i], aes(x=regime, y= value,
                                   ymin = lower, ymax = upper, 
                                   color = factor(coarsen), fill = factor(coarsen)))+
              geom_line()+geom_ribbon(alpha=0.2)+
