...
nmes <- c("ave.dat.A","ave.dat.Y","ave.hal.A",
            ...
            paste0("regest.pop.",reg.nmes),
            paste0("regSE.pop.",reg.nmes))
...
marginsA <- simp.res[,c(1,3,5,7,9)]
marginsY <- simp.res[,c(2,4,6,8,10)]

kp <- c("regest.dat.A","regSE.dat.A","regest.hal.A","regSE.hal.A",
        "regest.halcv.A","regSE.halcv.A","regest.sl.A","regSE.sl.A","regest.pop.A","regSE.pop.A") 
...
estim <- c(rep("data",2),rep("hal.und",2),rep("hal.cv",2),rep("SL",2),rep("pop",2))
err <- rep(c("bias2","var"),5)
...
covr <- c(covr, mean(apply(regrA[,9:10],1,in.CI,tru=psiP0[[4]][2])))


dat.cvr <- data.frame(estim = c("data","hal.und","hal.cv","SL","pop"),coverage=covr)
...
covr <- c(covr, mean(apply(tmle.res[,9:10],1,in.CI,tru=psiP0[[3]])))
