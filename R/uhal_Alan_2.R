# Alan adding SL version to data generation to existing HAL
# library(here)
# library(sl3)
...
Lrnr_uhal9001cv <- R6Class(
  classname = "Lrnr_uhal9001cv",
  inherit = Lrnr_base, portable = TRUE, class = TRUE,
  public = list(
    initialize = function(...) {
      params <- args_to_list()
...
fit_uhal <- function(X,
  ...
  # if empty model, skip undersmooth and output init
  if (dim(hal_init$basis_mat)[2] == 0) {
    x_basis <- hal_init$fit_init$x_basis
    lambda_under <- hal_init$fit_init$lambda
  }else{
...
fit_uhal_Qg_cv <- function(df, y_type, covars){
  task_Q <- sl3_Task$new(
    data = df,
    covariates = c(covars, "A"),
    outcome = "Y",
  ...
  Q_fit <- lrnr_uhalcv$train(task_Q)
  ...
  g_fit <- lrnr_uhalcv$train(task_g)
