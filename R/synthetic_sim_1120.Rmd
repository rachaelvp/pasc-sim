...
###Goal: Given the data in the simplified format, we want to know how to generate a SIMILAR simplified format data.


###DGP estimation
###Strategy:

##For the baseline covariates, we can take the empirical distributions.

#For the visit, we learn the conditional probability
#For the covid, we laern the conditional probability 
##For the vax, we learn the conditinal probability
##For the paxlovid, we learn the conditional probability
##For the metformin, we learn the conditional probability

##For the pasc, we learn the conditional intensity
##For the death, we learn the conditinoal intensity


###Implementation:

#Change from long to repeated for lasso
```{r}
...
##Fit the conditional probabilities using MAIN TERM LASSO, can also run HAL to actually be more flexible to the baseline covariates, 
#Prepare the data for conditional probabilities
```{r}
library(dplyr)
library(glmnet)

#load_all() in the console
result<-generate_data(1e3)
final<-result$final
study_tv<-result$study_tv
full<-TRUE
```
...
##Fit the conditional intensities using MAIN term lasso
#Prepare data for the conditional intensity estimation, up to 2 time points of history
...
```{r}
if(!full){
 coef(visit_fit,lambda=lambda.min) 
}
coef(covid_fit,lambda=lambda.min)
coef(vax_fit,lambda=lambda.min)
coef(metformin_fit,lambda=lambda.min)
coef(paxlovid_fit,lambda=lambda.min)
coef(death_fit,lambda=lambda.min)
coef(pasc_fit,lambda=lambda.min)
```




###Simulate from the estimated DGP

```{r}
sim_help<-function(temp_final,long_data_temp,t,full=TRUE){
...
sim_individual<-function(id){
    ...
    temp_result<-sim_help(temp_final,long_data_temp,t=1)
    ...
    if(death_happened==0){
        ...
        temp_result<-sim_help(temp_final,long_data_temp,t=2)
        long_data_temp<-temp_result[[2]]
        ...
        while(death_happened==0 & t_current<=15){
          ...
          temp_result<-sim_help(temp_final,long_data_temp,t=t_current)
          long_data_temp<-temp_result[[2]]
